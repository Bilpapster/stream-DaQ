...
CSV_OUTPUT_PATH = get_output_file_name(WINDOW_TYPE, WINDOW_DURATION_STR, SLIDE_DURATION_STR, GAP_DURATION_STR, NUM_CORES)  # Path inside the container
...
def get_output_file_name(window_type: str, window_duration: str, slide_duration: str | None, gap_duration: str | None, num_cores: str | int) -> str:
    window_type = str(window_type).to_lower
    match window_type:
        case "tumbling":
...
def main():
    ...
    consumer = KafkaConsumer(
        ...
        consumer_timeout_ms=10*60*1000  # Adjust as needed
    )
