...
from data_generator import create_input
...
def stringify(aggregate_result):
    ...
    (window_id, ((max, min, mean, count, distinct), window_metadata)) = aggregate_result
    timestamp = window_metadata.open_time
    return str(window_id), f"{timestamp},{window_id},{max},{min},{mean},{count},{distinct}"
...
stream = op.input("input", flow, TestingSource(create_input()))
...
op.output("persist_results", flattened_results, FileSink(Path("data.csv")))
